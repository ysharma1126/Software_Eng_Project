...
//  private void load_initial_cards() {
...
//  }
  ...
  /*
   * Submits the set to the server Gets response from the server Returns whether valid set or not
   */
//  private boolean submit_cards() {
...
//    for (Location location : locations_clicked) {
//      center_pane.getChildren().remove(location_to_node.get(location));
...
//  }
  ...
  public Game(Stage primaryStage, ObjectOutputStream outToServer, ObjectInputStream inFromServer,
    ...
    // load_initial_cards(outToServer, inFromServer, );
    ...
    // try {
    // server_response_handler = new Thread(new GameThread(primaryStage, outToServer, inFromServer,
    // set_correct, center_pane,
    // location_to_card, location_to_node, location_to_click_status, locations_clicked,
    // username_to_score_field));
